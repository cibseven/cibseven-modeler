...
describe('package', () => {
  ...
  describe('package.json', () => {
    it('has correct name and version', () => {
      ...
      expect(packageJson.name).toBe('cibseven-modeler', 'Package name is incorrect')
      expect(packageJson.version).toMatch(/^\d+\.\d+\.\d+(-.+)?$/, 'Package version is not a valid semver')
    })

    it('all dependencies have fixed versions', () => {
      const packageJsonPath = path.resolve(srcDir, '../package.json')
      const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, 'utf-8'))
      const dependencies = packageJson.dependencies || {}
      for (const [dep, version] of Object.entries(dependencies)) {
        expect(version).not.toMatch(/^[\^~]/, `Dependency ${dep} has a non-fixed version (${version})`)
      }
    })
  })
})
